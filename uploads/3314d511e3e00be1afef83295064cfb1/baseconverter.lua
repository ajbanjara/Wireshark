...
local function baseconverter_window()
    ...
    local function display_results(win, number)
        ...
        local function basen(n, b)
            ...
            --[[
                FIXME: For some reason we end up with numbers having a
                       large number of leading zeros, so delete them.
            --]]
            t = t:gsub("^0*", "")
            return sign .. t
        end -- basen()
        ...
        local function display_baseN_result(number)
            ...
            if base > 0 then
                text = basen(number:tonumber(), base)
                if lower_case == false then
                    text = string.upper(text)
                end
    ...
    win:add_button("Enter Number", function()
     ...
     win:add_button("Enter Base", function()
        ...
        local function get_base(inputstr)

            -----------------------------------------------------------------
            local function get_base_number(inputstr)
                ...
                if not numberstr then
                    win:set("Invalid base " .. base .. " number entered: " .. inputstr)
                    return
                end
            ...
            local basestr = string.match(inputstr, "^%d%d?$")
            if basestr then
                local userbase = tonumber(basestr)
                if userbase == 0 or (userbase >= 2 and userbase <= 36) then
                    base = userbase
                    if base >= 2 then
                        str = "Enter Base " .. userbase .. " Number:"
                        new_dialog("Enter Number", get_base_number, str)
                    else
                        display_results(win, saved_number)
                    end
                else
                    win:set("Invalid base entered: " .. inputstr)
                end
            else
                win:set("Invalid base entered: " .. inputstr)
            end
        ...
        new_dialog("Enter Base", get_base, "Enter Base (2-36 or 0):")
     end)
