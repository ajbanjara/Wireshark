...
static void eth_cell_renderer_text_init       (EthCellRendererText      *celltext);
static void eth_cell_renderer_text_class_init (EthCellRendererTextClass *class);
static void eth_cell_renderer_text_finalize   (GObject                  *object);

static void eth_cell_renderer_text_get_property  (GObject                  *object,
						  guint                     param_id,
						  GValue                   *value,
						  GParamSpec               *pspec);
static void eth_cell_renderer_text_set_property  (GObject                  *object,
						  guint                     param_id,
						  const GValue             *value,
						  GParamSpec               *pspec);
static void eth_cell_renderer_text_get_size   (GtkCellRenderer          *cell,
					       GtkWidget                *widget,
					       ...
					       gint                     *width,
					       gint                     *height);
static void eth_cell_renderer_text_render     (GtkCellRenderer          *cell,
					       GdkWindow                *window,
					       GtkWidget                *widget,
...
static GtkCellEditable *eth_cell_renderer_text_start_editing (GtkCellRenderer      *cell,
							      GdkEvent             *event,
							      GtkWidget            *widget,
							      const gchar          *path,
							      GdkRectangle         *background_area,
...
#define ETH_CELL_RENDERER_TEXT_PATH "eth-cell-renderer-text-path"

#define ETH_CELL_RENDERER_TEXT_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE ((obj), ETH_TYPE_CELL_RENDERER_TEXT, EthCellRendererTextPrivate))

typedef struct _EthCellRendererTextPrivate EthCellRendererTextPrivate;
struct _EthCellRendererTextPrivate
{
...
GType
eth_cell_renderer_text_get_type (void)
{
  ...
  if (!cell_text_type)
    {
      ...
      {
        sizeof (EthCellRendererTextClass),
	NULL,		/* base_init */
	NULL,		/* base_finalize */
        (GClassInitFunc) eth_cell_renderer_text_class_init,
	NULL,		/* class_finalize */
	NULL,		/* class_data */
        sizeof (EthCellRendererText),
	0,              /* n_preallocs */
        (GInstanceInitFunc) eth_cell_renderer_text_init,
      };

      cell_text_type =
	g_type_register_static (GTK_TYPE_CELL_RENDERER, "EthCellRendererText",
				&cell_text_info, 0);
    }
...
static void
eth_cell_renderer_text_init (EthCellRendererText *celltext)
{
...
static void
eth_cell_renderer_text_class_init (EthCellRendererTextClass *class)
{
  ...
  object_class->finalize = eth_cell_renderer_text_finalize;
  
  object_class->get_property = eth_cell_renderer_text_get_property;
  object_class->set_property = eth_cell_renderer_text_set_property;

  cell_class->get_size = eth_cell_renderer_text_get_size;
  cell_class->render = eth_cell_renderer_text_render;
  cell_class->start_editing = eth_cell_renderer_text_start_editing;
...
#define ADD_SET_PROP(propname, propval, nick, blurb) g_object_class_install_property (object_class, propval, g_param_spec_boolean (propname, nick, blurb, FALSE, G_PARAM_READABLE | G_PARAM_WRITABLE))
  ...
  text_cell_renderer_signals [EDITED] =
    g_signal_new ("edited",
		  ...
		  G_SIGNAL_RUN_LAST,
		  G_STRUCT_OFFSET (EthCellRendererTextClass, edited),
		  NULL, NULL,
  ...
  g_type_class_add_private (object_class, sizeof (EthCellRendererTextPrivate));
}

static void
eth_cell_renderer_text_finalize (GObject *object)
{
  EthCellRendererText *celltext = ETH_CELL_RENDERER_TEXT (object);
  EthCellRendererTextPrivate *priv;

  priv = ETH_CELL_RENDERER_TEXT_GET_PRIVATE (object);
...
static void
eth_cell_renderer_text_get_property (GObject        *object,
				     guint           param_id,
				     GValue         *value,
				     GParamSpec     *pspec)
{
...
static void
set_bg_color (EthCellRendererText *celltext,
              GdkColor            *color)
{
  if (color)
    {
...
static void
set_fg_color (EthCellRendererText *celltext,
              GdkColor            *color)
{
  if (color)
    {
...
static void
set_font_description (EthCellRendererText  *celltext,
...
static void
eth_cell_renderer_text_set_property (GObject      *object,
				     guint         param_id,
				     const GValue *value,
				     GParamSpec   *pspec)
{
...
/**
 * eth_cell_renderer_text_new:
 * 
 * Creates a new #EthCellRendererText. Adjust how text is drawn using
 ...
 **/
GtkCellRenderer *
eth_cell_renderer_text_new (void)
{
  return g_object_new (ETH_TYPE_CELL_RENDERER_TEXT, NULL);
}
...
static PangoLayout*
get_layout (EthCellRendererText *celltext,
            GtkWidget           *widget,
            ...
            GtkCellRendererState flags)
{
  ...
  priv = ETH_CELL_RENDERER_TEXT_GET_PRIVATE (celltext);
...
static void
eth_cell_renderer_text_get_size (GtkCellRenderer *cell,
				 GtkWidget       *widget,
				 GdkRectangle    *cell_area,
				 gint            *x_offset,
				 gint            *y_offset,
				 ...
				 gint            *height)
{
  EthCellRendererText *celltext = (EthCellRendererText *) cell;
  PangoRectangle rect;
  PangoLayout *layout;
  ...
  if (celltext->calc_fixed_height)
      ...
      if (height)
	{
	  *height = cell->height;
      ...
      if (width == NULL) {
		  g_warning("get height pre:%u", *height);
	return;
	  }
    }
...
static void
eth_cell_renderer_text_render (GtkCellRenderer      *cell,
			       GdkDrawable          *window,
			       GtkWidget            *widget,
			       GdkRectangle         *background_area,
			       GdkRectangle         *cell_area,
...

{
  ...
  g_warning("render height back:%u cell:%u expose:%u", 
	  background_area->height, cell_area->height, expose_area->height);

  eth_cell_renderer_text_get_size (cell, widget, cell_area, &x_offset, &y_offset, NULL, NULL);
...
static void
eth_cell_renderer_text_editing_done (GtkCellEditable *entry,
				     gpointer         data)
{
  const gchar *path;
  ...
  priv = ETH_CELL_RENDERER_TEXT_GET_PRIVATE (data);
  ...
  path = g_object_get_data (G_OBJECT (entry), ETH_CELL_RENDERER_TEXT_PATH);
...
static gboolean
eth_cell_renderer_text_focus_out_event (GtkWidget *entry,
		                        GdkEvent  *event,
					gpointer   data)
{
  eth_cell_renderer_text_editing_done (GTK_CELL_EDITABLE (entry), data);
...
eth_cell_renderer_text_start_editing (GtkCellRenderer      *cell,
				      GdkEvent             *event,
				      GtkWidget            *widget,
				      const gchar          *path,
				      GdkRectangle         *background_area,
...
{
  GtkWidget *entry;
  EthCellRendererText *celltext;
  EthCellRendererTextPrivate *priv;

  celltext = ETH_CELL_RENDERER_TEXT (cell);
  priv = ETH_CELL_RENDERER_TEXT_GET_PRIVATE (cell);
  ...
  g_object_set_data_full (G_OBJECT (entry), ETH_CELL_RENDERER_TEXT_PATH, g_strdup (path), g_free);
  ...
  g_signal_connect (entry,
		    "editing_done",
		    G_CALLBACK (eth_cell_renderer_text_editing_done),
		    celltext);
  priv->focus_out_id = g_signal_connect (entry, "focus_out_event",
		         G_CALLBACK (eth_cell_renderer_text_focus_out_event),
		         celltext);
...
/**
 * eth_cell_renderer_text_set_fixed_height_from_font:
 * @renderer: A #EthCellRendererText
 ...
 **/
void
eth_cell_renderer_text_set_fixed_height_from_font (EthCellRendererText *renderer,
						   gint                 number_of_rows)
{
  g_return_if_fail (ETH_IS_CELL_RENDERER_TEXT (renderer));
