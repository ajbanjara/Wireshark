...
-- https://gitlab.com/wireshark/wireshark/-/merge_requests/4225
local ipv6_sa_mac
if ws_version < "3.5.1" then
    ipv6_sa_mac = Field.new("ipv6.sa_mac")
else
    ipv6_sa_mac = Field.new("ipv6.slaac_mac")
end
...
function resolved_items_window()
    ...
    if ws_version < "3.5.1" then
        taps.ipv6 = Listener.new("frame", "ipv6.addr or ipv6.sa_mac")
    else
        taps.ipv6 = Listener.new("frame", "ipv6.addr or ipv6.slaac_mac")
    end
                ...
                for i in pairs(addr_ex) do
                    ...
                    if tapdata.mac_addr[addr_str] == nil then
                        ...
                        if string.find(mac_type, mt) == nil then
                            ...
                            makes the output more consistent when both eth.addr and
                            ipv6.sa_mac/ipv6.slaac_mac resolve to the same value.
                            --]]
            ...
            for i in pairs(sll_src_eth_ex) do
                ...
                if tapdata.mac_addr[src_eth[4]] == nil then
                    ...
                    if string.find(mac_type, "sll") == nil then
                        ...
                        makes the output more consistent when both eth.addr and
                        ipv6.sa_mac/ipv6.slaac_mac resolve to the same value,
                        so do the same for sll.src.eth too.
                        --]]
    ...
    function taps.ipv6.packet(pinfo, tvb)
        ...
        for i in pairs(ipv6_sa_mac_ex) do
            ...
            if tapdata.mac_addr[sa_mac[4]] == nil then
                ...
                if string.find(mac_type, "ipv6 slaac") == nil then
                    mac_type = mac_type .. ",ipv6 slaac"
                end
            end
