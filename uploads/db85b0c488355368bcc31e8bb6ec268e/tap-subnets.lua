--[[
    ...
    Copyright (C) 2020-2022 Christopher Maynard
...
function subnets_statistics_window()
    ...
    --[[
        NOTE: Prior to commit 21f57f04, get_filter() didn't work.
        See: https://gitlab.com/wireshark/wireshark/-/issues/17188
    --]]
    if get_version() < "3.7.0" then
        display_filter = "Not supported"
    else
        display_filter = get_filter() or "Not supported"
    end
    ...
    local tapwin = TextWindow.new(
        "Subnets Statistics (Tap #" .. instances .. ": " .. os.date() .. ", Filter: " .. display_filter .. ")"
    )
    tapwin:set("")
    ...
    local taps = {
        ip = Listener.new("frame",
            ((display_filter == "Not supported" and "") or display_filter), false)
    }
