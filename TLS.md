...
The TLS dissector is fully functional and even supports advanced features such as decryption of TLS if appropriate secrets are provided ([\#TLS\_Decryption](/TLS#TLS_Decryption)).
...
Wireshark supports TLS decryption when appropriate secrets are provided. The two available methods are:
  ...
  - Key log file using per-session secrets ([\#Using\_the\_.28Pre.29-Master-Secret](/TLS#Using_the_.28Pre.29-Master-Secret)).
...
The RSA private key file can only be used in the following circumstances:
  ...
  - The session has not been resumed. The handshake must include the [ClientKeyExchange](/ClientKeyExchange) handshake message.
...
See also [SampleCaptures\#SSL\_with\_decryption\_keys](/SampleCaptures#SSL_with_decryption_keys).
  ...
  - <https://github.com/wireshark/wireshark/blob/master/test/captures> - The test suite contains various TLS traces.
